...
def developer_earnings_summary(request):
    ...
    regular_qs = OrderItem.objects.filter(
    ...
    regular_qs = regular_qs.annotate(
        earnings=ExpressionWrapper(
            ...
            * Cast(F("product__percentage"), FloatField())
            / 100,
    ...
    monthly_data = {}
    for entry in (
        regular_qs.annotate(month=TruncMonth("order__created_at"))
        .values("month")
    ...
    for entry in (
        bulk_qs.annotate(month=TruncMonth("created_at"))
        .values("month")
        .annotate(total=Sum("earnings"))
    ):
        key = entry["month"].strftime("%b %Y")
    ...
    order_breakdown = [
        {
            "order_id": o["order_id"],
            "earnings": round(o["earnings"] or 0, 2),
            "type": "Regular",
        }
        for o in regular_orders
    ] + [
        {"order_id": o["id"], "earnings": round(o["earnings"] or 0, 2), "type": "Bulk"}
