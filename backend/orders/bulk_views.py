...
def create_bulk_order(request):
    ...
    if quantity < 10:
        return Response({"error": "The minimum bulk order quantity is 10."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        return Response({'error': 'Bulk ordering is not available for this product.'}, status=status.HTTP_400_BAD_REQUEST)

    if product.stock < quantity:
        return Response({'error': f'Insufficient stock. Only {product.stock} items left.'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    if designer_id:
        try:
            ...
            return Response({'error': 'Selected designer not found.'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    return Response({
        'message': 'Bulk order successfully created.',
