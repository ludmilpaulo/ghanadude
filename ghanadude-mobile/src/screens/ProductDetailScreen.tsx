...
const ProductDetailScreen: React.FC<ProductDetailProps> = ({ route }) => {
  ...
  const handleAddToCart = () => {
    ...
    const cartItem = {
      ...
      selectedSize,
      quantity,
      image: product.images?.[0]?.image || "", // Use first image, fallback to empty string
    };
  ...
  return (
    <SafeAreaView style={tw`flex-1 bg-gray-100`}>
      <ScrollView contentContainerStyle={tw`pb-10`}>
        ...
        {/* Add to Cart Button */}
        <TouchableOpacity
          style={tw`mt-6 ${selectedSize ? "bg-indigo-600" : "bg-gray-400"} rounded-xl py-4 items-center shadow-md flex-row justify-center`}
