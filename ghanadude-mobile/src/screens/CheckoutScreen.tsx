...
const CheckoutScreen: React.FC = () => {
  ...
  const calculateVAT = (amount: number) =>
    siteSettings
      ? parseFloat(((amount * siteSettings.vat_percentage) / 100).toFixed(2))
      : 0;
  ...
  const calculateDeliveryFee = async (): Promise<number> => {
    ...
    const isInternational =
      form.country.trim().toLowerCase() !== "south africa";
    ...
    const weightChargeKg =
      totalItems >= 10 ? Math.floor((totalItems - 5) / 5) : 0;
  ...
  const handlePaymentClose = async (reference?: string) => {
    ...
    try {
      ...
      if (bulkOrderId) {
        await fetch(
          `${API_BASE_URL}/order/bulk-orders/${bulkOrderId}/update-status/`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ status: "Processing" }),
  ...
  const autofillFromCurrentLocation = async () => {
    ...
    try {
      ...
      if (first) {
        ...
        const address =
          `${first.streetNumber || first.name || ""} ${first.street || ""}`.trim();
        const isoCode = first.isoCountryCode || "";

        let cleanedNumber = form.phone_number
          .replace(/[^0-9]/g, "")
          .replace(/^0+/, "");
    ...
    >
      <ScrollView contentContainerStyle={tw`p-4 pb-28`}>
        ...
        {orderType === "delivery" ? (
          ...
          <View style={tw`bg-yellow-100 p-4 rounded-lg my-4`}>
            <Text style={tw`text-yellow-800 font-semibold`}>
              🏬 You&apos;ll collect this order from our store. No need to enter
              shipping info.
            </Text>
          </View>
        )}
        <CheckoutSummary
          ...
          customDesignQty={
            design.customDesign ? design.customDesignQty || 1 : 0
          }
        />

        <TouchableOpacity
          onPress={initiatePayment}
          style={tw`bg-green-600 mt-6 p-4 rounded-lg`}
        >
