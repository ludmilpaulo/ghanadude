...
//export const API_BASE_URL = "http://192.168.100.212:8000";
...
const AuthService = {
  ...
  getUserProfile: async (userId: number) => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/account/account/profile/${userId}/`,
      );
      return response.data;
    } catch (error: unknown) {
      if (error instanceof AxiosError) {
  ...
  updateUserProfile: async (userId: number, data: object) => {
    try {
      const response = await axios.put(
        `${API_BASE_URL}/account/account/update/${userId}/`,
        data,
      );
      return response.data;
    } catch (error: unknown) {
  ...
  restoreAccount: async (userId: number) => {
    const res = await fetch(`${API_BASE_URL}/account/users/restore/`, {
      method: "POST",
