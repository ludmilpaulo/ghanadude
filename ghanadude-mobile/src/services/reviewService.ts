...
export const fetchProductReviews = async (productId: number, userId?: number) => {
...
export const createReview = async (productId: number, userId: number, rating: number, comment: string) => {
  const url = `${API_BASE_URL}/product/products/${productId}/review/`;
  const res = await axios.post(url, { user_id: userId, rating, comment });
  return res.data;
};

export const reactToReview = async (reviewId: number, userId: number, action: 'like' | 'dislike') => {
